...
class Linear(Module):
    def __init__(self, input_dim: int, output_dim: int, bias = True, init_weights = None):
        ...
        self.weights = tensor.tensor(self.init_weights((input_dim, output_dim)))

        if bias:
            self.bias = tensor.tensor(self.init_weights((output_dim,)))
        else:
    ...
    def forward(self, X):
        ...
        assert X.ndim == 2, f"Expected 2d array, found {X.ndim}d array"
